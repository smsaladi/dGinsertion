...
Options:
    ...
    -sub|--allow-sub        on|off    : whether allow subsequences, default=$allow_sub {DISABLED}
...
Modified local copy by Shyam Saladi, May 2012
...
=ss no sub allowed and must output to STDOUT
...
#ss if ($outformat =~ /^h/)
#ss {
 #ss   print $fpout  "<table cellspacing=\"0\" cellpadding=\"0\" border=\"0\" width=\"100%\" class=\"TableGrey\">\n";
...
#print $fpout  "<tr><td align=\"left\" colspan=4><big><big>Results</big></big><br><br><br><br><br></td></tr>\n";
    ...
    unless (scalar(@seq)) {
#ss        print $fpout  "<tr><td>No sequence entered!</td></tr></table><br><br><br><br>\n";
	print $fpout  "No sequence entered!\n";
        exit;
    }
...
#ss    print $fpout  "<tr><td colspan=4 align=left><table width=\"100%\" cellspacing=0 cellpadding=0 border=0>\n";
=ss subsequence not allowed in this version
...
#ss        ($helix =~ /^\s+$/) && (print $fpout  "<tr><td colspan=4>&nbsp;</td></tr>\n") && next;
        ($helix =~ /^\s+$/) && (print $fpout  "\n") && next;
        $helix =~ s/\s*$//;
        $helix =~ s/^\s*//;
        $helix =~ tr/a-z/A-Z/;
        unless ($helix =~ /^[ABCDEFGHIKLMNPQRSTUVWXYZ]+$/) {
#ss            print $fpout  "<tr><td>$helix</td><td>&nbsp;</td><td colspan=2>Sequence error. </td></tr>\n";
            print $fpout  "$helix\tSeqERR\n";
            next;
        }
        $Length=length($helix);
        if ($Length > 40) {
#ss            print $fpout  "<tr><td>" . substr($helix,0,40) . "... </td><td>&nbsp;</td><td colspan=2>Length error (> 40 residues)</td></tr>\n";
            print $fpout  substr($helix,0,40) . "\tLenERR\n";
            next;
        }
        if ($Length < 9) {
#ss            print $fpout  "<tr><td>$helix</td><td>&nbsp;</td><td colspan=2>Length error (< 9 residues)</td></tr>\n";
            print $fpout  "$helix\tLenERR\n";
            next;
        }
...
#ss        print $fpout  "<tr><td>$helix</td><td>&nbsp;</td><td>$helix_dG</td>";
	print $fpout  "$helix\t$helix_dG\n";
        $helix =~ s/^.*?\>//; $helix =~ s/<.*$//;
#ss        print $fpout  "<td><a href=\"analyze.php?with_length=$with_length&seq=$helix\" target=\"_blank\" style='text-decoration:none' onClick=\"javascript:analyze('$helix');return false;\"><span style='text-decoration: none;color: #800000;'>[analyze]</span></a></td></tr>\n";
    }
#ss    print $fpout  "</table><br><br><br><br></td></tr></table>\n";
    if ($fpout ne STDOUT)
    {
        close($fpout);
    }
...
=ss took out if else (everything is to and from console)
}
